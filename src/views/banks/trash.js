...
const trash = () => {
  ...
  const data = useSelector((state) => state.data?.trashBanks)
  ...
  const fetchData = async () => {
    try {
      ...
      if (performSearch) {
        ...
        response = await new BasicProvider(
          `banks/trash/all?page=${currentPage}&count=${count}`,
        ).getRequest()
      }
      dispatch({ type: 'set', data: { trashBanks: response.data.data } })
  ...
  const restoreBanks = async (userId) => {
    try {
      await new BasicProvider(`banks/multi/restore`).patchRequest({ ids: [userId] })
  ...
  const columns = [
    {
      name: 'Name',
      selector: (row) => <div className="data_table_colum">{row && row.name ? row.name : '-'}</div>,
    },
    {
      name: 'Created',
      cell: (row) => (
