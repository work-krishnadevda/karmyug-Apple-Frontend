...
import { cilPencil, cilZoom, cilSearch, cilTrash, cilCloudDownload } from '@coreui/icons'
...
const PenaltyList = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const usersWithPenalty = (res.data || []).filter(
        (user) =>
          Array.isArray(user.penalty_or_addon) &&
          user.penalty_or_addon.some((p) => p.type === 'penalty'),
      )

      let flatList = usersWithPenalty.flatMap((user) =>
      ...
      if (monthFilter) {
        flatList = flatList.filter((item) => item.date?.startsWith(monthFilter))
      }
  ...
  const downloadExcel = async () => {
    ...
    try {
      ...
      const worksheet = workbook.addWorksheet('Penalty Records')
      ...
      const fileName = `Penalty_Records_${monthFilter || 'All'}_${moment().format(
        'YYYY-MM-DD',
      )}.xlsx`
  ...
  return (
    <CCard className="p-3">
      <CCardHeader className="d-flex justify-content-between align-items-center">
        <h5 className="mb-0">Penalty Records</h5>
        <CButton
          color="success"
      ...
      <CCardBody>
        ...
        {loading && <AppTableSkeleton ariaLabel="Loading penalty records" rows={6} />}
      </CCardBody>

      {/* -------- VIEW MODAL ---------- */}
      <CModal visible={viewModal} onClose={() => setViewModal(false)}>
        <CModalHeader>
          <CModalTitle>View Penalty</CModalTitle>
        </CModalHeader>
        <CModalBody>
          {selected && (
            <>
              ...
              <p>
                <strong>Date:</strong> {new Date(selected.date).toDateString()}
              </p>
              {selected.createdAt && (
                <p>
      ...
      <CModal visible={editModal} onClose={() => setEditModal(false)}>
        <CModalHeader>
          <CModalTitle>Edit Penalty</CModalTitle>
        </CModalHeader>
        <CModalBody>
          {selected && (
            <>
        ...
        <CModalFooter>
          ...
          <CButton color="primary" onClick={updatePenalty}>
            Save
          </CButton>
        </CModalFooter>
      </CModal>
      ...
      <CModal visible={deleteModal} onClose={() => setDeleteModal(false)}>
        <CModalHeader>
          <CModalTitle>Delete Penalty</CModalTitle>
        </CModalHeader>
        <CModalBody>
          <p>Are you sure you want to delete this penalty record?</p>
          {selected && (
            <>
              <p>
                <strong>Employee:</strong> {selected.employee?.name}
        ...
        <CModalFooter>
          ...
          <CButton color="danger" onClick={deletePenalty}>
            Delete
          </CButton>
        </CModalFooter>
      </CModal>
