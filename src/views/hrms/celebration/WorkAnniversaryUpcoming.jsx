...
const WorkAnniversaryUpcoming = () => {
  const dispatch = useDispatch()
  const [anniversaryData, setAnniversaryData] = useState([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState(null)

  // Fetch upcoming work anniversary data from API
  const fetchAnniversaryData = async () => {
    try {
      ...
      const response = await new BasicProvider(
        'celebration/work-anniversary/upcoming',
        dispatch,
      ).getRequest()
      ...
      console.error('Error fetching upcoming work anniversary data:', err)
      setError(err?.response?.data?.message || 'Failed to fetch upcoming work anniversary data')
      setAnniversaryData([])
      toast.error('Failed to load upcoming work anniversary data')
    } finally {
      setLoading(false)
    }
  }
  ...
  if (loading) {
    return (
      <div className="p-4">
        <AppContentSkeleton variant="cards" cards={4} ariaLabel="Loading upcoming work anniversaries" />
      </div>
    )
  }
    ...
    return (
      <div className="p-4">
        <div className="bg-slate-50 border border-slate-200 rounded-lg p-8 text-center">
          <p className="text-slate-600 text-lg">📅 No upcoming work anniversaries!</p>
  ...
  return (
    <div className="p-4">
      <div className="mb-6 text-center">
        ...
        <p className="text-slate-600">Get ready to celebrate with our team members!</p>
      </div>
...
export default WorkAnniversaryUpcoming
