...
export default function ReAssignToAnotherDM() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (performSearch) {
        ...
        queryData['status'] = 'pending for draft'
        ...
        response = await new BasicProvider(
          `cases/all-cases-reassign/dm?page=${currentPage}&count=${count}`,
        ).getRequest()
      }
  ...
  const columns = [
    ...
    },
    {
      ...
      selector: (row) => (
        <div className="data_table_colum">
          <div className="">{row?.dm?.name ? row.dm?.name : '-'}</div>
        </div>
      ),
      // width: '10%',
    },
  ...
  return (
    <>
      <SingleSubHeader moduleName={'All Cases Re-Assign to Another DM'}
