...
export default function Pages() {
  ...
  const data = useSelector((state) => state.data?.trashCustomers)
    ...
    try {
      ...
      if (performSearch) {
        ...
        response = await new BasicProvider(
          `customers/trash/search?${HelperFunction.convertToQueryString(queryData)}`,
        ).getRequest()
      } else {
        response = await new BasicProvider(
          `customers/trash/all?page=${currentPage}&count=${count}`,
        ).getRequest()
      }
      dispatch({ type: 'set', data: { trashCustomers: response.data.data } })
  ...
  useEffect(() => {
    const fetchSelectedRows = async () => {
      const savedSelectedRows = await handleSelectedRowChange('trashcustomers')
  ...
  const restore = async (userId) => {
    try {
      await new BasicProvider(`customers/multi/restore`).patchRequest({ ids: [userId] })
          ...
          <div className="datatable">
            <DataTable
              ...
              onChangeRowsPerPage={(value) => {
                ...
                setSelectedRowForModule('trashcustomers', value)
              }}
