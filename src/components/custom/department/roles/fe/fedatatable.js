import { cilCheckAlt, cilPencil, cilSpreadsheet, cilTrash } from '@coreui/icons'
...
export default function FeDataTable() {
  ...
  const [concurnResolutionVisible, setConcurnResolutionVisible] = useState(false)
  ...
  const [concernReasonVisible, setConcernReasonVisible] = useState(false)
  ...
  const handleOfflineVisitDone = async (id) => {
    try {
      let data = {
        status: 'visit done',
      }
        ...
        <div className="data_table_colum">
          <div className="fs-12 pt-1">
            {row?.created_at && moment(row.created_at).isValid() ? (
              <CTooltip content={moment(row.created_at).format('DD MMM YYYY hh:mm:ss A')}>
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="action-btn">
          {row.concern_resolution &&
            (row.status == 'updated by coo' ||
              row.status == 'updated by bm' ||
              row.status == 'updated by admin' ||
              row.status == 'updated by sfo') && (
              <div
                onClick={() => {
                  setCaseId(row._id)
              ...
              [
                ...
                <>
                  <CButton
                    ...
                    color="info"
                    onClick={() =>
                      navigate(`/case/${row._id}/edit?isTie=true`, {
                        state: { id: row._id, isTie: true },
                      })
                    }
                  >
                    View
                  ...
                  {[
                    ...
                    <CustomTooltip content={'Concern Reason!'}>
                      <div
                        onClick={() => {
                          setCaseId(row._id)
                          setConcernReasonVisible(!concernReasonVisible)
                        }}
                        className="edit-btn pointer_cursor"
                      >
                        <FontAwesomeIcon icon={faHandshakeSlash} style={{ color: 'red' }} />
                      </div>
                    </CustomTooltip>
                  )}
                </>
          ...
          {query.get('data') == 'pending_for_accept' && (
            <div>
  ...
  return (
    <>
          ...
          <div className="datatable">
            <DataTable
              conditionalRowStyles={[
                ...
                {
                  when: (row) => row.status === 'updated by sfo',
                  style: {
                    // backgroundColor: '#AFE1AF',
                    fontWeight: '800',
                    // transition:'.5s ease'
      ...
      <ConcurnResolutio
        visible={concurnResolutionVisible}
        close={() => setConcurnResolutionVisible(false)}
        caseId={caseId}
      />
      ...
      <ConcernReason
        visible={concernReasonVisible}
        close={() => setConcernReasonVisible(false)}
        caseId={caseId}
      />
