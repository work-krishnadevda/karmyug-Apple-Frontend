...
export default function FE_Concurn() {
    ...
    const fetchData = async () => {
        try {
            ...
            if (performSearch) {
                ...
                response = await new BasicProvider(`cases?concern=fe-concern&page=${currentPage}&count=${count}`).getRequest()
            }
                        ...
                        <p className="rounded-pill mb-0 text-capitalize">
                            ...
                            >
                                <>
                                    {row.status
                                        .toLowerCase()
                                        .replace(/\b(coo|bm|fe|sfo)\b/g, (match) => match.toUpperCase())}
                                </>
                            </CBadge>
                        </p>
                    ) : (
        ...
        },
        {
            ...
            cell: (row) => (
                <div className="action-btn">
                    {row.concern_resolution && (row.status == 'updated by coo' || row.status == 'updated by admin' || row.status == 'updated by bm' || row.status == 'updated by sfo') && (
                        <div
                            onClick={() => {
                                setCaseId(row._id)
                    ...
                    <div className="datatable">
                        <DataTable
                            conditionalRowStyles={[
                                ...
                                {
                                    when: (row) => row.status === 'updated by admin',
                                    style: {
                                        // backgroundColor: '#AFE1AF',
                                        fontWeight: '800',
                                        // transition:'.5s ease'
