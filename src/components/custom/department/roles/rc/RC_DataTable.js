    ...
    try {
      ...
      if (performSearch) {
        ...
        response = await new BasicProvider(
          ...
          `cases?page=${currentPage}&count=${count}`,
  ...
  const columns = [
    ...
    },
    {
      ...
      selector: (row) => (
        <div
          onClick={() => {
            if (loggedinUser._id === row?.rc?._id) {
          ...
          <div className="datatable">
            <DataTable
              conditionalRowStyles={[
                {
                  when: (row) => row?.send_back_logs?.length > 0 && row?.status === 'pending for rc' && row?.send_back_logs.some(e => e.by === loggedInUser._id),
                  style: {
                    fontWeight: '800',
                  },
                },
