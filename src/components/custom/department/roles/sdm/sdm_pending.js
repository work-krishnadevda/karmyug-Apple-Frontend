...
export default function SDM_Pending() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (performSearch) {
        ...
        response = await new BasicProvider(`cases/pending?page=${currentPage}&count=${count}`).getRequest()
      }
  ...
  return (
    ...
    <>
      <SingleSubHeader moduleName={'Assign Pending'}
                  ...
                  <CCol md={6} className="d-flex align-iten-center justify-content-end">
                    <div>
...

                      {
                        initialValues.dm && (
                          <CButton
                            className="add_new" onClick={sendToDM}>
                            Assign For Draft
                          </CButton>
                        )
                      }
