...
export default function AdminOverTAT() {
    ...
    const fetchData = async () => {
        try {
            ...
            if (performSearch) {
                ...
                queryData['overtat'] = true //for a loop and overtat comes from backend as written in backend api
                response = await new BasicProvider(
                    `cases/search?${HelperFunction.convertToQueryString(queryData)}`,
                ).getRequest()
            } else {
                response = await new BasicProvider(`cases?page=${currentPage}&count=${count}&overtat=24`).getRequest()
            }
    ...
    const columns = [
                    ...
                    {row && row.status ? (
                        <p className="rounded-pill mb-0 text-capitalize">
                            <CBadge
                                ...
                                //                                     ? '#73b43c'
                                //                                     : ['hold by coo', 'hold by sdm', 'hold by dm'].includes(row.status)
                                //                                       ? 'red'
                                //                                       : ' ',
                                // }}
                                style={{
        ...
        //       {
        //         // Show Hold button when not hovered and status is 'hold by coo'
        ...
        //       {hoveredRows[row._id] === 'holded' && row.status === 'hold by coo' && (
        //         <CButton
        //           onClick={() => {
        //             setCaseId(row._id)
