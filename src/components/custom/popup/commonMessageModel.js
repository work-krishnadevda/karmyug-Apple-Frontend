...
export const CommonMessageShowModel = (props) => {
  const location = useLocation()
  const query = new URLSearchParams(location.search)
  let { caseId, handleAcceptCase, setVisibleConfirmAcc, isShowCopyBtn = true } = props
        ...
        <CModalBody className="bg-theme b-0 ">
          <div className="message_table">
            <CTable className="">
              ...
              <CTableBody>
                ...
                <CTableRow>
                  ...
                  <CTableDataCell className="font-small-size-data">
                    {caseValue && moment(caseValue?.date_initiation_bank).format('DD MMM YYYY hh:mm:ss A')
                      ? moment(caseValue?.date_initiation_bank).format('DD MMM YYYY hh:mm:ss A')
                      : '-'}
                  </CTableDataCell>
                </CTableRow>
            ...
            <p style={{
              ...
              whiteSpace: 'normal',
              maxWidth: '100%',
            }} >Final Address : {caseValue?.final_address ? caseValue.final_address : '-'}</p>

          </div>
        </CModalBody>

        <CModalFooter>
          {caseValue?.status == 'pending for accept' && loggedinUserRole && loggedinUserRole?.name == process.env.REACT_APP_FE && (
            <CButton
              onClick={async () => {
